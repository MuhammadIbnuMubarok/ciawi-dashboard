...
async function rtTick(){const b=document.getElementById("live-badge");try{const r=await fetch(rtBase()+encodeURIComponent(RT_SOURCE),{cache:"no-store"});if(!r.ok)throw new Error("HTTP "+r.status);const buf=await r.arrayBuffer();const u8=new Uint8Array(buf);let txt;
...
}catch(e){console.warn("RT offline: "+e.message);if(b){b.textContent="HULU TERPUTUS";b.className="text-[10px] px-2 py-0.5 rounded bg-siaga-2/15 text-siaga-2 border border-siaga-2/40 font-bold";}}}
