/* APP LAYER - logika UI: render tabel, filter, chart, dry dam, jam server */
const HARI=["Min","Sen","Sel","Rab","Kam","Jum","Sab"];function hariISO(iso){return HARI[new Date(iso+"T00:00:00").getDay()];}
const $=id=>document.getElementById(id);
const fmt2=x=>x==null?"—":x.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2});
const MONTHS=["Jan","Feb","Mar","Apr","Mei","Jun","Jul","Agu","Sep","Okt","Nov","Des"];
const tglID=iso=>(+iso.slice(8,10))+" "+MONTHS[+iso.slice(5,7)-1]+" "+iso.slice(0,4);
const stamp=r=>tglID(r.tanggal)+" "+r.jam;
let selectedYears=new Set(),selectedMonths=new Set();
let workingDataset=[...REAL_DATA];
let currentPage=1,pageSize=12,currentInterval="1h",currentPreset="all",tableMinimized=false,currentChart="flood-jul2025";
const CHART_SCENARIOS={
'flood-jul2025':{labels:["06 Jul 01:00","06 Jul 02:00","06 Jul 03:00","06 Jul 04:00","06 Jul 05:00","06 Jul 06:00","06 Jul 07:00","06 Jul 08:00","06 Jul 09:00","06 Jul 10:00","06 Jul 11:00","06 Jul 12:00","06 Jul 13:00","06 Jul 14:00","06 Jul 15:00","06 Jul 16:00","06 Jul 17:00","06 Jul 18:00","06 Jul 19:00","06 Jul 20:00"],
elevations:[532.20,530.00,528.00,526.00,524.00,522.00,521.00,520.00,519.00,518.50,518.00,517.50,517.00,516.60,516.00,515.50,515.00,518.50,521.00,521.55],
volumes:[1245315.48,950714.14,742915.27,559514.96,398244.43,275278.78,223903.42,177289.03,137313.57,121380.06,105446.55,93218.89,80991.23,73291.03,61740.73,53774.64,45808.54,121380.06,223903.42,252159.87],
inflows:Array(20).fill(null),outflows:Array(20).fill(null),qNull:true,
sedimen:[9.20,9.20,9.20,9.20,9.20,9.20,9.20,9.20,12.70,12.70,12.20,12.20,11.70,11.20,10.70,10.20,9.70,9.70,9.70,9.70],
summary:"REKOR DATASET: Elv. 532,20 m • Vol. 1.245.315,48 m³ (06 Jul 2025 01:00 WIB) — telemetri debit tidak terekam"},
'flood-mar2025':{labels:["15 Mar 01:00","15 Mar 04:00","15 Mar 08:00","15 Mar 12:00","15 Mar 15:00","15 Mar 16:00","15 Mar 16:30","15 Mar 16:50","15 Mar 17:33"],
elevations:[517.08,516.51,516.06,515.87,520.85,522.38,522.82,522.82,522.63],
volumes:[82947.66,71558.48,62895.76,59669.55,216911.26,296596.56,321280.30,321280.30,310621.41],
inflows:Array(9).fill(null),outflows:Array(9).fill(null),qNull:true,
sedimen:[10.20,10.20,10.20,10.20,10.20,10.20,10.20,10.20,10.20],
summary:"Puncak: Elv. 522,82 m • Vol. 321.280,30 m³ (15 Mar 2025 16:30) — Q tidak terekam"},
'flood-dec2025':{labels:["05 Des 00:00","05 Des 01:00","05 Des 02:00","05 Des 03:00","05 Des 04:00","05 Des 05:00","05 Des 07:00","05 Des 08:00","05 Des 09:00","05 Des 10:00","05 Des 11:00","05 Des 12:00"],
elevations:[509.16,509.16,509.16,509.16,509.16,509.16,509.16,509.16,509.16,509.16,509.16,511.00],
volumes:[2059.11,2059.11,2059.11,2059.11,2059.11,2059.11,2059.11,2059.11,2059.11,2059.11,2059.11,8620.58],
inflows:[9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,59.68],
outflows:[9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,9.40,57.86],
sedimen:[2.70,2.70,2.70,2.70,2.70,2.70,2.70,2.70,2.70,2.70,2.70,2.70],
summary:"Siaga IV: Q In 59,68 vs Q Out 57,86 m³/s → Reduksi 1,82 m³/s (05 Des 12:00)"},
'flood-feb2026':{labels:["16 Feb 22:00","16 Feb 23:00","17 Feb 00:00","17 Feb 01:00","17 Feb 02:00","17 Feb 03:00"],
elevations:[510.10,510.30,510.70,510.50,510.30,510.00],
volumes:[4635.28,5520.90,7292.15,6406.53,5520.90,4192.47],
inflows:[12.91,13.74,26.19,21.56,17.26,14.59],outflows:[12.91,13.74,26.19,21.56,17.26,14.59],
sedimen:[3.50,3.50,3.50,3.50,3.50,3.50],
summary:"Siaga IV singkat: puncak Q 26,19 m³/s (17 Feb 00:00), reduksi 0,00 m³/s"},
'flood-apr2026':{labels:["20 Apr 00:00","20 Apr 01:00","20 Apr 02:00","20 Apr 03:00","20 Apr 04:00","20 Apr 05:00"],
elevations:[508.67,508.55,508.49,508.44,508.39,508.37],
volumes:[1252.56,1107.03,1034.26,973.63,912.99,888.73],
inflows:[20.57,15.90,13.32,12.50,12.50,11.69],outflows:[20.57,15.90,13.32,12.50,12.50,11.69],
sedimen:[3.50,3.50,3.50,3.50,3.50,3.50],
summary:"Pasca-Siaga IV: surut terkendali 508,67 → 508,37 m"},
'all-overview':{labels:["Des 2024","Mar 2025","Jul 2025 (REKOR)","Sep 2025","Des 2025","Jan 2026","Feb 2026","Mar 2026","Apr 2026","Jul 2026"],
elevations:[511.15,522.82,532.20,509.69,513.10,510.00,510.70,509.89,509.62,505.75],
volumes:[9568.27,321280.30,1245315.48,3405.16,24123.73,4192.47,7292.15,3913.10,3227.38,179.48],
inflows:[null,null,null,null,26.73,20.57,26.19,13.32,10.91,7.31],outflows:[null,null,null,null,26.73,20.57,26.19,13.32,10.91,7.31],
sedimen:[4.70,10.20,12.70,3.90,4.20,3.50,3.50,3.50,3.50,0.80],
summary:"Rekor multi-tahun: Elv. 532,20 m / 1.245.315,48 m³ (Jul 2025)"}
};
const PHASES={
1:{title:"(1) Kondisi Air Normal Hingga Awal Terjadinya Banjir",desc:"Aliran sungai masuk melalui konduit bawah dan langsung diteruskan ke hilir tanpa ditahan; waduk kering sebagaimana desain dry dam.",waterPath:"M 230,260 L 688,260 L 688,252 L 230,252 Z",waterY:245,waterText:"Aliran Normal Bebas",conduitH:12,inflowWidth:5,splashScale:0.6,badge:"FASE 1 DARI 4: NORMAL"},
2:{title:"(2) Kondisi Banjir Puncak",desc:"Setiap volume air yang masuk melebihi kapasitas konduit akan tertahan sementara di waduk, memangkas debit puncak secara signifikan.",waterPath:"M 230,260 L 688,260 L 688,105 L 230,105 Z",waterY:110,waterText:"Muka Air Tampungan Meningkat",conduitH:20,inflowWidth:14,splashScale:1.4,badge:"FASE 2 DARI 4: BANJIR PUNCAK"},
3:{title:"(3) Kondisi Sesaat Setelah Banjir (Penyusutan)",desc:"Debit inflow menurun; air yang tertahan perlahan dialirkan melalui konduit hingga muka air surut terkendali.",waterPath:"M 230,260 L 688,260 L 688,170 L 230,170 Z",waterY:165,waterText:"Tampungan Menyusut Terkendali",conduitH:16,inflowWidth:8,splashScale:1.0,badge:"FASE 3 DARI 4: PENYUSUTAN"},
4:{title:"(4) Kondisi Pasca Banjir",desc:"Ketika banjir usai dan seluruh volume air yang tertahan telah dilepas, waduk kembali kering siap menampung banjir berikutnya.",waterPath:"M 230,260 L 688,260 L 688,256 L 230,256 Z",waterY:250,waterText:"Waduk Kering Kembali Semula",conduitH:10,inflowWidth:4,splashScale:0.5,badge:"FASE 4 DARI 4: PASCA BANJIR"}
};
function statusBadge(s){if(s==null)return '<span class="px-2 py-0.5 rounded border border-outline-variant/40 text-on-surface-variant text-[10px]">TIDAK TERCATAT</span>';const c={"Normal":"siaga-normal","Siaga IV":"siaga-4","Siaga III":"siaga-3","Siaga II":"siaga-2","Siaga I":"siaga-1","Flushing":"secondary"}[s]||"on-surface-variant";return '<span class="px-2 py-0.5 rounded border border-'+c+'/40 bg-'+c+'/10 text-'+c+' text-[10px] font-bold">'+s.toUpperCase()+'</span>';}
function qbar(v,mx,c){return '<div class="flex-1 '+c+' rounded-t" style="height:'+Math.max(2,v==null?3:(v/mx)*100)+'%" title="'+(v==null?"tidak terekam":fmt2(v))+'"></div>';}
function applyFilters(){const q=$("table-search").value.trim().toLowerCase();const ds=$("date-range-start").value,de=$("date-range-end").value;let d=getBaseDataset().filter(r=>{if(!selectedYears.has(r.tanggal.slice(0,4)))return false;if(!selectedMonths.has(r.tanggal.slice(5,7)))return false;if(ds&&r.tanggal<ds)return false;if(de&&r.tanggal>de)return false;if(currentPreset==="siaga"&&!(r.status&&r.status.indexOf("Siaga")===0))return false;if(currentPreset==="flushing"&&!((r.status==="Flushing")||(r.sedimen!=null&&r.sedimen<=1.0)))return false;if(q){const hay=(r.no+" "+r.tanggal+" "+r.jam+" "+r.elevasi+" "+(r.status||"")).toLowerCase();if(hay.indexOf(q)===-1)return false;}return true;});const WIN={"5m":5,"15m":15,"30m":30,"1h":60,"6h":360,"24h":1440,"7d":10080}[currentInterval]||60;const __now=Date.now();d=d.filter(r=>{const t=new Date(r.tanggal+"T"+r.jam).getTime();return (__now-t)<=WIN*60000;});d.sort((a,b)=>((a.tanggal+"T"+a.jam)<(b.tanggal+"T"+b.jam)?1:-1));workingDataset=d;currentPage=1;renderTable();}
function filterTableBySearch(){applyFilters();}
function filterTablePreset(p){currentPreset=p;["all","siaga","flushing"].forEach(k=>$("tbl-btn-"+k).classList.toggle("active",k===p));applyFilters();}
function setIntervalSampling(v){currentInterval=v;["5m","15m","30m","1h","6h","24h","7d"].forEach(k=>$("int-"+k).classList.toggle("active",k===v));applyFilters();}
function resetDateRangeFilter(){$("date-range-start").value="2024-12-01";$("date-range-end").value="2026-07-31";applyFilters();}
function changePageSize(v){pageSize=v==="all"?100000:+v;currentPage=1;renderTable();}
function goToPage(n){currentPage=n;renderTable();}
function goToPrevPage(){if(currentPage>1){currentPage--;renderTable();}}
function goToNextPage(){currentPage++;renderTable();}
function renderTable(){const total=workingDataset.length;const pages=Math.max(1,Math.ceil(total/pageSize));if(currentPage>pages)currentPage=pages;const start=(currentPage-1)*pageSize;const rows=workingDataset.slice(start,start+pageSize);$("telemetry-tbody").innerHTML=rows.map(r=>"<tr class='hover:bg-surface-container-low/60 transition-colors'><td class='py-2 px-3 text-on-surface-variant'>"+r.no+"</td><td class='py-2 px-3'>"+tglID(r.tanggal)+"</td><td class='py-2 px-3'>"+r.jam+"</td><td class='py-2 px-3 text-right text-primary font-semibold'>"+fmt2(r.elevasi)+"</td><td class='py-2 px-3 text-right'>"+fmt2(r.sedimen)+"</td><td class='py-2 px-3 text-right'>"+fmt2(r.bukaan)+"</td><td class='py-2 px-3 text-right text-secondary'>"+fmt2(r.vol)+"</td><td class='py-2 px-3 text-right'>"+fmt2(r.qout_konduit)+"</td><td class='py-2 px-3 text-right'>"+fmt2(r.qout_spillway)+"</td><td class='py-2 px-3 text-right'>"+fmt2(r.qout_total)+"</td><td class='py-2 px-3 text-right text-siaga-2'>"+fmt2(r.qin)+"</td><td class='py-2 px-3 text-right text-siaga-normal'>"+fmt2(r.reduksi)+"</td><td class='py-2 px-3 text-center'>"+statusBadge(r.status)+"</td></tr>").join("")||"<tr><td colspan='13' class='py-6 text-center text-on-surface-variant'>Tidak ada data yang cocok dengan filter.</td></tr>";$("tbl-rowcount").textContent="Menampilkan "+(total?start+1:0)+"–"+Math.min(start+pageSize,total)+" dari "+total+" rekaman";$("tbl-badge-summary").textContent=total+" REKAMAN AKTIF";let pg="";const win=[];for(let p=1;p<=pages;p++){if(p===1||p===pages||Math.abs(p-currentPage)<=1)win.push(p);else if(win[win.length-1]!=="…")win.push("…");}$("tbl-pagination").innerHTML=win.map(p=>p==="…"?'<span class="px-1 text-on-surface-variant">…</span>':'<button class="w-7 h-7 rounded text-xs '+(p===currentPage?"bg-primary-container text-on-primary-container font-bold":"bg-surface-container text-on-surface-variant hover:text-on-surface")+'" onclick="goToPage('+p+')">'+p+"</button>").join("");$("btn-prev-page").disabled=currentPage<=1;$("btn-next-page").disabled=currentPage>=pages;}
function exportTableToCSV(filename){const head=["NO","TANGGAL","JAM","ELEVASI_M","SEDIMEN_M","BUKAAN_KONDUIT_M","VOLUME_M3","QOUT_KONDUIT","QOUT_SPILLWAY","QOUT_TOTAL","QIN","REDUKSI","STATUS"];const lines=[head.join(",")].concat(workingDataset.map(r=>[r.no,r.tanggal,r.jam,r.elevasi,r.sedimen,r.bukaan==null?"":r.bukaan,r.vol,r.qout_konduit==null?"":r.qout_konduit,r.qout_spillway==null?"":r.qout_spillway,r.qout_total==null?"":r.qout_total,r.qin==null?"":r.qin,r.reduksi==null?"":r.reduksi,r.status==null?"":r.status].join(",")));const blob=new Blob([lines.join("\r\n")],{type:"text/csv;charset=utf-8;"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=filename;a.click();URL.revokeObjectURL(a.href);}
function downloadFilteredData(){exportTableToCSV("neraca_ciawi_terfilter_"+new Date().toISOString().slice(0,10)+".csv");}
function setDatasetFilter(key){currentChart=key;document.querySelectorAll(".chart-filter-btn").forEach(b=>b.classList.remove("active"));const b=$("filter-"+key);if(b)b.classList.add("active");renderChart(key);}
function selectDataPoint(i){const d=CHART_SCENARIOS[currentChart];const qi=d.inflows[i],qo=d.outflows[i];const rd=(qi!=null&&qo!=null)?qi-qo:null;$("chart-cursor-info").innerHTML="<b class='text-primary'>"+d.labels[i]+"</b> • Elv. <b>"+fmt2(d.elevations[i])+" m</b> • Vol. <b>"+fmt2(d.volumes[i])+" m³</b> • Q In <b>"+(qi==null?"tidak terekam":fmt2(qi)+" m³/s")+"</b> • Q Out <b>"+(qo==null?"tidak terekam":fmt2(qo)+" m³/s")+"</b> • Reduksi <b>"+(rd==null?"—":fmt2(rd)+" m³/s")+"</b>";}
function renderBars(d){const el=$("inflow-outflow-bars");if(d.qNull){el.innerHTML='<div class="w-full h-full flex items-center justify-center text-[11px] text-siaga-3 border border-dashed border-siaga-3/40 rounded p-2 text-center">Telemetri debit (Q In / Q Out) tidak terekam pada periode ini — hanya elevasi & volume tersedia di NERACA.csv.</div>';return;}const mx=Math.max.apply(null,d.inflows.concat(d.outflows).filter(v=>v!=null))||1;el.innerHTML=d.labels.map((lb,i)=>{const qi=d.inflows[i],qo=d.outflows[i],rd=(qi!=null&&qo!=null)?qi-qo:null;return '<div class="flex-1 h-full flex items-end gap-[2px] cursor-pointer" onclick="selectDataPoint('+i+')" title="'+lb+'">'+qbar(qi,mx,"bg-siaga-2")+qbar(qo,mx,"bg-primary")+qbar(rd,mx,"bg-siaga-normal")+"</div>";}).join("");}
function renderSedimenBars(d){const mx=Math.max.apply(null,d.sedimen)||1;$("sedimen-bars").innerHTML=d.sedimen.map((s,i)=>'<div class="flex-1 h-full flex items-end cursor-pointer" onclick="selectDataPoint('+i+')" title="'+d.labels[i]+'"><div class="w-full bg-tertiary-container rounded-t" style="height:'+Math.max(2,(s/mx)*100)+'%"></div></div>').join("");$("sedimen-summary").textContent="Sedimen maks periode ini: "+fmt2(mx)+" m • ambang perhatian > 10,00 m";}
function renderChart(key){const d=CHART_SCENARIOS[key];if(!d||!d.elevations||!d.elevations.length){["chart-grid","chart-dots-group","chart-xlabels"].forEach(id=>{const e=$(id);if(e)e.innerHTML="";});["chart-elevation-line","chart-volume-line","chart-volume-area"].forEach(id=>{const e=$(id);if(e)e.setAttribute("d","");});const ib=$("inflow-outflow-bars");if(ib)ib.innerHTML="";const sb=$("sedimen-bars");if(sb)sb.innerHTML="";const mc=$("chart-margin-chip");if(mc)mc.textContent="MARGIN KE PELIMPAH: —";const ci=$("chart-cursor-info");if(ci)ci.textContent="Belum ada data ditampilkan - centang tahun dan bulan pada panel filter untuk menyalakan grafik & tabel.";return;}const n=d.elevations.length;const lo=Math.min.apply(null,d.elevations),hi=Math.max.apply(null,d.elevations);const minElv=lo-2;const maxElv=hi+((hi-lo)<8?2.5:6);const maxVol=(Math.max.apply(null,d.volumes)*1.15)||1;const X=i=>70+i*((860-70)/Math.max(1,n-1));const EY=v=>200-((Math.max(minElv,Math.min(maxElv,v))-minElv)/(maxElv-minElv))*175;const VY=v=>200-Math.min(1,v/maxVol)*175;let g="";for(let i=0;i<=4;i++){const y=200-i*43.75;const elv=minElv+(maxElv-minElv)*i/4;const vol=maxVol*i/4;g+='<line x1="70" y1="'+y+'" x2="860" y2="'+y+'" stroke="#222a3d" stroke-width="1"/>';g+='<text x="64" y="'+(y+3)+'" fill="#bcc9cd" font-size="9" font-family="JetBrains Mono" text-anchor="end">'+elv.toFixed(1)+"</text>";g+='<text x="866" y="'+(y+3)+'" fill="#38BDF8" font-size="9" font-family="JetBrains Mono">'+(vol>=1000000?(vol/1000000).toFixed(1)+" jt":vol>=1000?Math.round(vol/1000)+" rb":vol.toFixed(0))+"</text>";}if(546.75>=minElv&&546.75<=maxElv){const y=EY(546.75);g+='<line x1="70" y1="'+y+'" x2="860" y2="'+y+'" stroke="#FBBF24" stroke-dasharray="5 4" stroke-width="1.2"/><text x="72" y="'+(y-4)+'" fill="#FBBF24" font-size="9" font-family="JetBrains Mono">ELV. PELIMPAH 546,75 m</text>';}$("chart-grid").innerHTML=g;let ep="",vp="",va="M "+X(0)+",200";for(let i=0;i<n;i++){ep+=(i?" L ":"M ")+X(i)+","+EY(d.elevations[i]);vp+=(i?" L ":"M ")+X(i)+","+VY(d.volumes[i]);va+=" L "+X(i)+","+VY(d.volumes[i]);}va+=" L "+X(n-1)+",200 Z";$("chart-elevation-line").setAttribute("d",ep);$("chart-volume-line").setAttribute("d",vp);$("chart-volume-area").setAttribute("d",va);let dots="";for(let i=0;i<n;i++){dots+='<circle cx="'+X(i)+'" cy="'+EY(d.elevations[i])+'" r="4.5" fill="#0b1326" stroke="#4cd7f6" stroke-width="2" style="cursor:pointer" onclick="selectDataPoint('+i+')"><title>'+d.labels[i]+"</title></circle>";}$("chart-dots-group").innerHTML=dots;let xl="";const step=Math.max(1,Math.ceil(n/4));for(let i=0;i<n;i+=step){xl+='<text x="'+X(i)+'" y="218" fill="#bcc9cd" font-size="9" font-family="JetBrains Mono" text-anchor="middle">'+d.labels[i]+"</text>";}if((n-1)%step!==0){xl+='<text x="'+X(n-1)+'" y="218" fill="#bcc9cd" font-size="9" font-family="JetBrains Mono" text-anchor="middle">'+d.labels[n-1]+"</text>";}$("chart-xlabels").innerHTML=xl;$("chart-margin-chip").textContent="MARGIN KE PELIMPAH: "+fmt2(546.75-hi)+" m";$("metric-reduksi-summary").textContent=d.summary;renderBars(d);renderSedimenBars(d);$("chart-cursor-info").textContent="Klik titik data pada grafik untuk membaca elevasi, Q In, dan reduksi per timestamp.";}
function setDryDamPhase(n){const p=PHASES[n];$("dam-water-path").setAttribute("d",p.waterPath);$("water-level-text").setAttribute("y",p.waterY-6);$("water-level-text").textContent=p.waterText;$("water-level-line").setAttribute("y2",p.waterY);$("dam-conduit").setAttribute("y",258-p.conduitH);$("dam-conduit").setAttribute("height",p.conduitH);$("dam-outflow").setAttribute("y",260-p.conduitH);$("dam-outflow").setAttribute("height",Math.max(4,p.conduitH-4));$("dam-splash").setAttribute("rx",14*p.splashScale);document.querySelectorAll("#dam-inflow path").forEach(el=>el.setAttribute("stroke-width",p.inflowWidth));$("phase-title").textContent=p.title;$("phase-desc").textContent=p.desc;$("phase-badge").textContent=p.badge;document.querySelectorAll(".phase-btn").forEach((b,i)=>b.classList.toggle("active",i===n-1));}
function scrollToSection(id){const el=$(id);if(el)el.scrollIntoView({behavior:"smooth",block:"start"});}
function handleNavClick(btn,id){document.querySelectorAll(".nav-item").forEach(b=>{b.className="nav-item text-xs px-3 py-1.5 text-on-surface-variant hover:text-on-surface transition-all rounded";});btn.className="nav-item text-xs px-3 py-1.5 transition-all bg-primary-container text-on-primary-container font-bold rounded shadow-sm";scrollToSection(id);}
function toggleCollapseSection(bodyId,btnId){$(bodyId).classList.toggle("hidden");$(btnId+"-icon").classList.toggle("rotate-180");}
function toggleTablePanel(){tableMinimized=!tableMinimized;$("table-body-wrapper").classList.toggle("hidden",tableMinimized);$("btn-toggle-table-icon").classList.toggle("rotate-180",tableMinimized);$("btn-toggle-table-text").textContent=tableMinimized?"EXPAND":"MINIMIZE";}
function initStats(){const d=REAL_DATA;const latest=d[d.length-1];const maxElv=d.reduce((m,r)=>r.elevasi>m.elevasi?r:m,d[0]);const maxVol=d.reduce((m,r)=>r.vol>m.vol?r:m,d[0]);const wq=d.filter(r=>r.qin!=null);const maxQin=wq.reduce((m,r)=>r.qin>m.qin?r:m,wq[0]);const maxRed=wq.reduce((m,r)=>(r.reduksi||0)>(m.reduksi||0)?r:m,wq[0]);const counts={};d.forEach(r=>{const k=r.status||"Tidak tercatat";counts[k]=(counts[k]||0)+1;});$("stat-total").textContent=d.length+" rekaman";$("stat-period").textContent=stamp(d[0])+" → "+stamp(latest);$("stat-maxelv").innerHTML=fmt2(maxElv.elevasi)+" m<br><span class='text-[9px] text-on-surface-variant font-normal'>"+stamp(maxElv)+"</span>";$("stat-maxvol").innerHTML=fmt2(maxVol.vol)+" m³<br><span class='text-[9px] text-on-surface-variant font-normal'>"+stamp(maxVol)+"</span>";$("stat-maxqin").innerHTML=fmt2(maxQin.qin)+" m³/s<br><span class='text-[9px] text-on-surface-variant font-normal'>"+stamp(maxQin)+"</span>";$("stat-maxred").innerHTML=fmt2(maxRed.reduksi)+" m³/s<br><span class='text-[9px] text-on-surface-variant font-normal'>"+stamp(maxRed)+"</span>";$("stat-status-chips").innerHTML=Object.keys(counts).map(k=>'<span class="px-2 py-0.5 rounded bg-surface-container text-[10px]">'+k+": <b>"+counts[k]+"</b></span>").join("");$("strip-status").textContent=(latest.status||"—").toUpperCase();$("strip-count").textContent=d.length;$("strip-peak").textContent="ELV. "+fmt2(maxElv.elevasi)+" m ("+stamp(maxElv).toUpperCase()+" WIB)";$("strip-lastsync").textContent=stamp(latest).toUpperCase()+" WIB";$("kpi-elv").textContent=fmt2(latest.elevasi);$("kpi-elv-sub").textContent="TERKINI: "+stamp(latest).toUpperCase()+" WIB";$("kpi-vol").textContent=fmt2(latest.vol);$("kpi-qout").textContent=fmt2(latest.qout_total);$("kpi-qout-sub").textContent="BUKAAN KONDUIT "+fmt2(latest.bukaan)+" m • SPILLWAY "+fmt2(latest.qout_spillway);$("kpi-qin").textContent=fmt2(maxQin.qin);$("kpi-qin-sub").textContent="REDUKSI MAKS "+fmt2(maxRed.reduksi)+" m³/s • "+stamp(maxRed).toUpperCase();$("app-footer").textContent="© 2026 BBWS CILIWUNG-CISADANE — DASHBOARD DRY DAM CIAWI • DATASET RIIL NERACA.csv ("+d.length+" REKAMAN) • "+tglID(d[0].tanggal).toUpperCase()+" – "+tglID(latest.tanggal).toUpperCase();}
function startClock(){const f=new Intl.DateTimeFormat("en-GB",{timeZone:"Asia/Jakarta",year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false});const tick=()=>{const o={};f.formatToParts(new Date()).forEach(p=>{o[p.type]=p.value;});$("live-clock").textContent=o.year+"-"+o.month+"-"+o.day+" "+o.hour+":"+o.minute+":"+o.second+" WIB";};tick();setInterval(tick,1000);}
document.addEventListener("DOMContentLoaded",function(){initStats();applyFilters();setDatasetFilter("all-overview");setDryDamPhase(1);startClock();});


function buildOverview(){const src=(typeof getBaseDataset==="function")?getBaseDataset():REAL_DATA;const by={};src.forEach(r=>{if(r==null||r.elevasi==null)return;const k=r.tanggal.slice(0,7);if(!by[k]||r.elevasi>by[k].elevasi)by[k]=r;});const keys=Object.keys(by).sort();return {labels:keys.map(k=>hariISO(by[k].tanggal)+" "+tglID(by[k].tanggal)+" "+by[k].jam),elevations:keys.map(k=>by[k].elevasi),volumes:keys.map(k=>by[k].vol),inflows:keys.map(k=>by[k].qin),outflows:keys.map(k=>by[k].qout_total),sedimen:keys.map(k=>by[k].sedimen==null?0:by[k].sedimen),summary:"Puncak elevasi per bulan - dihitung otomatis dari seluruh rekaman NERACA + live",qNull:false};}
CHART_SCENARIOS["all-overview"]=buildOverview();

/* init dipindah ke blok bawah */
for(let i=1;i<=12;i++)selectedMonths.add(String(i).padStart(2,"0"));
function monthCounts(){const src=(typeof getBaseDataset==="function")?getBaseDataset():REAL_DATA;const c={};src.forEach(r=>{const m=r.tanggal.slice(5,7);c[m]=(c[m]||0)+1;});return c;}
function renderMonthChecks(){const box=document.getElementById("month-checks");if(!box)return;const c=monthCounts();let h="";for(let i=1;i<=12;i++){const m=String(i).padStart(2,"0");h+='<label class="flex items-center gap-1 text-[11px] bg-surface-container px-2 py-1 rounded cursor-pointer hover:bg-surface-container-high"><input type="checkbox" '+(selectedMonths.has(m)?"checked":"")+' onchange="toggleMonth(\''+m+'\',this.checked)" class="rounded bg-surface-container-lowest border-outline-variant/60"><span class="text-on-surface font-bold">'+MONTHS[i-1]+'</span><span class="text-on-surface-variant">('+(c[m]||0)+')</span></label>';}box.innerHTML=h;}
function toggleMonth(m,on){if(on)selectedMonths.add(m);else selectedMonths.delete(m);applyFilters();if(typeof currentChart!=="undefined"&&currentChart==="all-overview")renderChart("all-overview");}
function setAllMonths(all){selectedMonths=new Set();if(all){for(let i=1;i<=12;i++)selectedMonths.add(String(i).padStart(2,"0"));}renderMonthChecks();applyFilters();if(typeof currentChart!=="undefined"&&currentChart==="all-overview")renderChart("all-overview");}

/* deklarasi selectedYears dipindah ke baris atas */
function yearCounts(){const src=(typeof getBaseDataset==="function")?getBaseDataset():REAL_DATA;const c={};src.forEach(r=>{const y=r.tanggal.slice(0,4);c[y]=(c[y]||0)+1;});return c;}
function renderYearChecks(){const box=document.getElementById("year-checks");if(!box)return;const c=yearCounts();const years=Object.keys(c).sort();box.innerHTML=years.map(y=>'<label class="flex items-center gap-1 text-[11px] bg-surface-container px-2 py-1 rounded cursor-pointer hover:bg-surface-container-high"><input type="checkbox" '+(selectedYears.has(y)?"checked":"")+' onchange="toggleYear(\''+y+'\',this.checked)" class="rounded bg-surface-container-lowest border-outline-variant/60"><span class="text-on-surface font-bold">'+y+'</span><span class="text-on-surface-variant">('+c[y]+')</span></label>').join("");}
function toggleYear(y,on){if(on)selectedYears.add(y);else selectedYears.delete(y);applyFilters();if(typeof currentChart!=="undefined"&&currentChart==="all-overview")renderChart("all-overview");}
function setAllYears(all){const c=yearCounts();selectedYears=new Set(all?Object.keys(c):[]);renderYearChecks();applyFilters();if(typeof currentChart!=="undefined"&&currentChart==="all-overview")renderChart("all-overview");}
function inFilter(r){return selectedYears.has(r.tanggal.slice(0,4))&&selectedMonths.has(r.tanggal.slice(5,7));}
function buildEvent(from,to){const src=(typeof getBaseDataset==="function")?getBaseDataset():REAL_DATA;const rows=src.filter(r=>r.tanggal>=from&&r.tanggal<=to&&inFilter(r)).sort((x,z)=>((x.tanggal+"T"+x.jam)<(z.tanggal+"T"+z.jam)?-1:1));if(!rows.length)return null;return {labels:rows.map(r=>hariISO(r.tanggal)+" "+tglID(r.tanggal)+" "+r.jam),elevations:rows.map(r=>r.elevasi),volumes:rows.map(r=>r.vol),inflows:rows.map(r=>r.qin),outflows:rows.map(r=>r.qout_total),sedimen:rows.map(r=>r.sedimen==null?0:r.sedimen),qNull:rows.every(r=>r.qin==null),summary:"Puncak Elv. "+fmt2(Math.max.apply(null,rows.map(r=>r.elevasi)))+" m - dihitung langsung dari NERACA"};}
function refreshAllScenarios(){const ev={"flood-jul2025":["2025-07-06","2025-07-06"],"flood-mar2025":["2025-03-15","2025-03-15"],"flood-dec2025":["2025-12-05","2025-12-05"],"flood-feb2026":["2026-02-16","2026-02-17"],"flood-apr2026":["2026-04-20","2026-04-20"]};Object.keys(ev).forEach(k=>{const s=buildEvent(ev[k][0],ev[k][1]);if(s)CHART_SCENARIOS[k]=s;});CHART_SCENARIOS["all-overview"]=buildOverview();}
function buildOverview(){const src=((typeof getBaseDataset==="function")?getBaseDataset():REAL_DATA).filter(r=>selectedYears.has(r.tanggal.slice(0,4))&&selectedMonths.has(r.tanggal.slice(5,7)));const by={};src.forEach(r=>{if(r==null||r.elevasi==null)return;const k=r.tanggal.slice(0,7);if(!by[k]||r.elevasi>by[k].elevasi)by[k]=r;});const keys=Object.keys(by).sort();return {labels:keys.map(k=>hariISO(by[k].tanggal)+" "+tglID(by[k].tanggal)+" "+by[k].jam),elevations:keys.map(k=>by[k].elevasi),volumes:keys.map(k=>by[k].vol),inflows:keys.map(k=>by[k].qin),outflows:keys.map(k=>by[k].qout_total),sedimen:keys.map(k=>by[k].sedimen==null?0:by[k].sedimen),summary:"Puncak elevasi per bulan (tahun & bulan tercentang) - otomatis dari NERACA + live",qNull:false};}
selectedYears=new Set(Object.keys(yearCounts()));
refreshAllScenarios();
document.addEventListener("DOMContentLoaded",function(){renderYearChecks();renderMonthChecks();applyFilters();if(typeof currentChart!=="undefined"&&currentChart==="all-overview")renderChart("all-overview");});






function onFilterChange(){refreshAllScenarios();applyFilters();updateTrendLabel();setDatasetFilter("all-overview");}
function updateTrendLabel(){const allY=Object.keys(yearCounts());const ys=[...selectedYears].sort();const ms=[...selectedMonths].sort();let t=(ys.length===0?"BELUM ADA PILIHAN":(ys.length===allY.length?"TREN MULTI-TAHUN":(ys.length===1?"TAHUN "+ys[0]:"TREN "+ys.join(", "))));if(ms.length&&ms.length<12){t+=" • "+(ms.length<=3?ms.map(m=>MONTHS[+m-1]).join(", "):ms.length+" BULAN");}const b=document.getElementById("filter-all-overview");if(b)b.textContent=t;}
function toggleYear(y,on){on?selectedYears.add(y):selectedYears.delete(y);onFilterChange();renderYearChecks();}
function toggleMonth(m,on){on?selectedMonths.add(m):selectedMonths.delete(m);onFilterChange();renderMonthChecks();}
function setAllYears(all){selectedYears=new Set(all?Object.keys(yearCounts()):[]);onFilterChange();renderYearChecks();}
function setAllMonths(all){selectedMonths=new Set();if(all){for(let i=1;i<=12;i++)selectedMonths.add(String(i).padStart(2,"0"));}onFilterChange();renderMonthChecks();}
document.addEventListener("DOMContentLoaded",function(){updateTrendLabel();});





function updateDamLive(r){const y=Math.max(62,280-((r.elevasi-504.20)/(550.39-504.20))*220);const ln=document.getElementById("wl-live-line");if(ln){ln.setAttribute("y1",y);ln.setAttribute("y2",y);}const lb=document.getElementById("wl-live-label");if(lb){lb.setAttribute("y",y-5);lb.textContent="WL LIVE: "+r.elevasi.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m";}const st=document.getElementById("dam-live-status");if(st){st.textContent="KONDISI LIVE: "+(r.status||"—").toUpperCase()+" • ELV "+r.elevasi.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m • QOUT "+r.qoutT.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m³/s";}}
function updateDamLive(r){const tIn=r.elevasi-504.20;const tOut=r.F;const y=Math.max(62,280-((r.elevasi-504.20)/(550.39-504.20))*220);const ln=document.getElementById("wl-live-line");if(ln){ln.setAttribute("y1",y);ln.setAttribute("y2",y);}const lb=document.getElementById("wl-live-label");if(lb){lb.setAttribute("y",y-5);lb.textContent="TMA INLET LIVE: "+tIn.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m";}const lo=document.getElementById("tma-out-label");if(lo){lo.textContent="TMA OUTLET LIVE: "+tOut.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m";}const st=document.getElementById("dam-live-status");if(st){st.textContent="KONDISI LIVE: "+(r.status||"—").toUpperCase()+" • TMA IN "+tIn.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m • TMA OUT "+tOut.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m • QOUT "+r.qoutT.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m³/s";}}

function updateDamLive(r){const tIn=r.elevasi-504.20;const tOut=r.F;const y=Math.max(62,Math.min(258,252-((r.elevasi-505.70)/44.69)*190));
const ln=document.getElementById("wl-live-line");if(ln){ln.setAttribute("y1",y);ln.setAttribute("y2",y);}const lb=document.getElementById("wl-live-label");
if(lb){lb.setAttribute("y",y-6);lb.textContent="TMA INLET LIVE: "+tIn.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m";}
const lo=document.getElementById("tma-out-label");if(lo){lo.textContent="TMA OUTLET LIVE: "+tOut.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m";}
const st=document.getElementById("dam-live-status");if(st){st.textContent="KONDISI LIVE: "+(r.status||"—").toUpperCase()+" • TMA IN "+tIn.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m • TMA OUT "+tOut.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m • QOUT "+r.qoutT.toLocaleString("id-ID",{minimumFractionDigits:2,maximumFractionDigits:2})+" m³/s";}}


function buildOverview(){
const rows=baseData().filter(r=>selectedYears.has(r.tanggal.slice(0,4))&&selectedMonths.has(r.tanggal.slice(5,7)));
const by={};const daily=(selectedMonths.size===1);
rows.forEach(r=>{if(r.elevasi==null)return;const k=daily?r.tanggal:r.tanggal.slice(0,7);if(!by[k]||r.elevasi>by[k].elevasi)by[k]=r;});
const keys=Object.keys(by).sort();
return {labels:keys.map(k=>hariISO(by[k].tanggal)+" "+tglID(by[k].tanggal)+" "+by[k].jam),
elevations:keys.map(k=>by[k].elevasi),volumes:keys.map(k=>by[k].vol),
inflows:keys.map(k=>by[k].qin),outflows:keys.map(k=>by[k].qout_total),
sedimen:keys.map(k=>by[k].sedimen==null?0:by[k].sedimen),
summary:daily?"GRAFIK HARIAN PENUH 1 BULAN - puncak elevasi per hari (bulan tercentang)":"Puncak elevasi per bulan (tahun & bulan tercentang) - otomatis dari NERACA + live",qNull:false};
}
function baseData(){return (typeof getBaseDataset==="function")?getBaseDataset():REAL_DATA;}
function applyTheme(t){const L=t==="light";document.documentElement.classList.toggle("light",L);document.documentElement.classList.toggle("dark",!L);const ic=document.getElementById("theme-icon");if(ic)ic.textContent=L?"light_mode":"dark_mode";try{localStorage.setItem("ciawi-theme",t);}catch(e){}}
function toggleTheme(){const cur=document.documentElement.classList.contains("light")?"light":"dark";applyTheme(cur==="light"?"dark":"light");}
document.addEventListener("DOMContentLoaded",function(){let t="dark";try{t=localStorage.getItem("ciawi-theme")||"dark";}catch(e){}applyTheme(t);});

function sampleByMinutes(d){const MIN={"5m":1,"15m":1,"30m":1,"1h":1,"6h":5,"24h":15,"7d":60}[currentInterval]||1;d.sort((a,b)=>((a.tanggal+"T"+a.jam)<(b.tanggal+"T"+a.jam)<(b.tanggal+"T"+b.jam)?-1:1));if(MIN>1){const kept=[];let last=null;d.forEach(r=>{const t=new Date(r.tanggal+"T"+r.jam).getTime();if(last===null||t-last>=MIN*60000){kept.push(r);last=t;}});d=kept;}return d;}


function todayISO(){const o={};new Intl.DateTimeFormat("en-GB",{timeZone:"Asia/Jakarta",year:"numeric",month:"2-digit",day:"2-digit",hour12:false}).formatToParts(new Date()).forEach(p=>{o[p.type]=p.value;});return o.year+"-"+o.month+"-"+o.day;}
function resetDateRangeFilter(){const t=todayISO();const ds=$("date-range-start");const de=$("date-range-end");if(ds)ds.value=t;if(de)de.value=t;applyFilters();}
document.addEventListener("DOMContentLoaded",function(){const t=todayISO();const ds=$("date-range-start");const de=$("date-range-end");if(ds)ds.value=t;if(de)de.value=t;applyFilters();});
function renderBars(d){const el=$("#inflow-outflow-bars");if(!el)return;
if(d.qNull){el.innerHTML='<div class="text-xs text-on-surface-variant py-6">Tidak ada data Q pada skenario ini.</div>';return;}
const mx=Math.max.apply(null,d.inflows.concat(d.outflows).filter(v=>v!=null))||1;const n=d.labels.length;const step=Math.max(1,Math.ceil(n/6));
const bars=d.labels.map((L,i)=>{const qi=d.inflows[i],qo=d.outflows[i],rd=(qi!=null&&qo!=null)?Math.max(0,qi-qo):null;
return '<div class="flex-1 flex flex-col justify-end" style="height:100%" onclick="selectDataPoint('+i+')"><div class="w-full flex items-end justify-center gap-[2px]" style="height:100%">'+
'<div title="Q In" style="height:'+Math.max(2,(qi||0)/mx*100)+'%" class="w-1/3 bg-[#ff8a3d] rounded-t"></div><div title="Q Out" style="height:'+Math.max(2,(qo||0)/mx*100)+'%" class="w-1/3 bg-primary rounded-t"></div>'+
'<div title="Reduksi" style="height:'+Math.max(2,(rd||0)/mx*100)+'%" class="w-1/3 bg-[#35e0a1] rounded-t"></div></div></div>';}).join("");
const labs=d.labels.map((L,i)=>'<div class="flex-1 text-[9px] leading-tight text-on-surface-variant whitespace-nowrap'+(i%step===0?'':' invisible')+'">'+L+'</div>').join("");
el.innerHTML='<div class="w-full flex flex-col" style="height:100%"><div class="flex items-end gap-1" style="height:80%">'+bars+'</div><div class="flex gap-1 pt-1 border-t border-outline-variant/30 mt-1">'+labs+'</div></div>';}
function tipBox(){let t=document.getElementById("hover-tip");if(!t){t=document.createElement("div");t.id="hover-tip";t.style.cssText="position:fixed;z-index:9999;pointer-events:none;display:none;background:rgba(11,19,38,.95);border:1px solid rgba(76,215,246,.4);border-radius:6px;padding:6px 8px;font-size:11px;line-height:1.5;color:#e6edf7;max-width:280px;box-shadow:0 6px 18px rgba(0,0,0,.5);";document.body.appendChild(t);}return t;}
function tipShow(html,x,y){const t=tipBox();t.innerHTML=html;t.style.display="block";const w=t.offsetWidth,h=t.offsetHeight;let L=x+14,T=y+14;if(L+w>innerWidth-8)L=x-w-14;if(T+h>innerHeight-8)T=y-h-14;t.style.left=L+"px";t.style.top=T+"px";}
function tipHide(){const t=document.getElementById("hover-tip");if(t)t.style.display="none";}
function crossHost(h){if(getComputedStyle(h).position==="static")h.style.position="relative";return h;}
function crossShow(h,x,y){let v=h.querySelector(".xc-v"),g=h.querySelector(".xc-h");if(!v){v=document.createElement("div");v.className="xc-v";v.style.cssText="position:absolute;pointer-events:none;top:0;bottom:0;width:0;border-left:1px dashed rgba(76,215,246,.55);display:none;";h.appendChild(v);}
if(!g){g=document.createElement("div");g.className="xc-h";g.style.cssText="position:absolute;pointer-events:none;left:0;right:0;height:0;border-top:1px dashed rgba(76,215,246,.35);display:none;";h.appendChild(g);}v.style.left=x+"px";v.style.display="block";if(y==null){g.style.display="none";}else{g.style.top=y+"px";g.style.display="block";}}
function crossHide(h){if(!h)return;const v=h.querySelector(".xc-v"),g=h.querySelector(".xc-h");if(v)v.style.display="none";if(g)g.style.display="none";}
function trendSvg(){if(window.__tsvg&&document.contains(window.__tsvg))return window.__tsvg;let best=null,bn=0;document.querySelectorAll("svg").forEach(sv=>{const c=sv.querySelectorAll("circle").length;if(c>bn){bn=c;best=sv;}});window.__tsvg=best;return best;}
function fmtN(v,u){return v==null?"—":(+v).toLocaleString("id-ID",{maximumFractionDigits:2})+(u||"");}
function tipTrend(i){const d=CHART_SCENARIOS[currentChart];if(!d)return "";const red=(d.inflows[i]!=null&&d.outflows[i]!=null)?Math.max(0,d.inflows[i]-d.outflows[i]):null;return "<b>"+d.labels[i]+"</b><br>Elevasi: "+fmtN(d.elevations[i]," m")+"<br>Volume: "+fmtN(d.volumes[i]," m³")+"<br>Q In: "+fmtN(d.inflows[i]," m³/s")+"<br>Q Out: "+fmtN(d.outflows[i]," m³/s")+"<br>Reduksi: "+fmtN(red," m³/s")+"<br>Sedimen: "+fmtN(d.sedimen[i]," m");}
function hoverTrend(e){const sv=trendSvg();if(!sv)return false;const r=sv.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom){crossHide(sv.parentElement);return false;}
const cs=Array.from(sv.querySelectorAll("circle"));if(!cs.length)return false;let bi=0,bd=1e9;cs.forEach((c,ix)=>{const cr=c.getBoundingClientRect();const dx=Math.abs((cr.left+cr.right)/2-e.clientX);if(dx<bd){bd=dx;bi=ix;}});
const hr=sv.parentElement.getBoundingClientRect();const cr=cs[bi].getBoundingClientRect();crossHost(sv.parentElement);crossShow(sv.parentElement,(cr.left+cr.right)/2-hr.left,(cr.top+cr.bottom)/2-hr.top);tipShow(tipTrend(bi),e.clientX,e.clientY);return true;}
function hoverBars(e){const b=document.getElementById("inflow-outflow-bars");if(!b)return false;const r=b.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom){crossHide(b);return false;}
const row=b.querySelector(".items-end")||b;const gs=Array.from(row.children);if(!gs.length)return false;let bi=0;gs.forEach((g,ix)=>{const gr=g.getBoundingClientRect();if(e.clientX>=gr.left&&e.clientX<=gr.right)bi=ix;});
const gr=gs[bi].getBoundingClientRect();const hr2=b.getBoundingClientRect();crossHost(b);crossShow(b,(gr.left+gr.right)/2-hr2.left,null);tipShow(tipTrend(bi),e.clientX,e.clientY);return true;}
document.addEventListener("mousemove",function(e){if(hoverTrend(e))return;if(hoverBars(e))return;tipHide();});
window.addEventListener("blur",function(){tipHide();const sv=trendSvg();if(sv)crossHide(sv.parentElement);crossHide(document.getElementById("inflow-outflow-bars"));});

const SIAGA_F=[0.64,1.86,2.55,3.40];const SIAGA_TXT=["SIAGA I","SIAGA II","SIAGA III"];const SIAGA_COL={1:"#38BDF8",2:"#FBBF24",3:"#F97316",4:"#EF4444"};
function siagaZone(f){return f<=SIAGA_F[0]?0:f<=SIAGA_F[1]?1:f<=SIAGA_F[2]?2:f<=SIAGA_F[3]?3:4;}
function siagaHost(){const p=document.getElementById("dam-water-path");if(!p)return null;const sv=p.closest("svg");const h=sv?sv.parentElement:null;if(h&&getComputedStyle(h).position==="static")h.style.position="relative";return h;}
function siagaOverlays(){const h=siagaHost();if(!h)return null;let box=document.getElementById("siaga-box");if(!box){box=document.createElement("div");box.id="siaga-box";box.style.cssText="position:absolute;left:10px;top:10px;z-index:5;font-size:11px;line-height:1.6;pointer-events:none;";h.appendChild(box);}
let g=document.getElementById("siaga-gauge");if(!g){g=document.createElement("div");g.id="siaga-gauge";g.style.cssText="position:absolute;right:10px;bottom:10px;z-index:5;font-size:9px;pointer-events:none;";h.appendChild(g);}return {box:box,g:g};}
function siagaRender(f){const o=siagaOverlays();if(!o)return;const z=siagaZone(f==null?0:f);
o.box.innerHTML=SIAGA_TXT.map(function(t,i){const zone=4-i;const on=z===zone;return '<div style="color:'+(on?SIAGA_COL[zone]:"#64748b")+';font-weight:'+(on?"800":"600")+';opacity:'+(on?"1":"0.55")+'">'+t+(on?" ◀ AKTIF":"")+'</div>';}).join("")+'<div style="color:#e6edf7;margin-top:2px">AKTIF: '+(z===0?"NORMAL":z===1?"SIAGA IV":SIAGA_TXT[4-z])+'</div>';
const segs=[[0,SIAGA_F[0],"#334155"],[SIAGA_F[0],SIAGA_F[1],"#38BDF8"],[SIAGA_F[1],SIAGA_F[2],"#FBBF24"],[SIAGA_F[2],SIAGA_F[3],"#F97316"],[SIAGA_F[3],4,"#EF4444"]];const W=180;const fx=(f==null?0:Math.min(4,Math.max(0,f)))/4*W;
o.g.innerHTML='TMA OUTLET (m) - patokan<div style="position:relative;width:'+W+'px;height:8px;border-radius:4px;overflow:hidden;margin:2px 0 1px">'+segs.map(function(s){return '<span style="position:absolute;left:'+(s[0]/4*W)+'px;width:'+((s[1]-s[0])/4*W)+'px;top:0;bottom:0;background:'+s[2]+'"></span>';}).join("")+'<span style="position:absolute;left:'+(fx-1)+'px;top:-2px;bottom:-2px;width:2px;background:#fff"></span></div>';
o.g.innerHTML+=' <div style="position:relative;width:'+W+'px;height:10px;color:#94a3b8">'+SIAGA_F.map(function(v){return '<span style="position:absolute;left:'+(v/4*W-8)+'px">'+v.toFixed(2).replace(".",",")+'</span>';}).join("")+'</div><div style="color:#e6edf7">F sekarang: '+(f==null?"—":f.toFixed(2).replace(".",","))+' m</div>';
const ol=document.getElementById("tma-out-label");if(ol)ol.setAttribute("fill",z===0?"#94a3b8":SIAGA_COL[z]);}
const _udl=updateDamLive;updateDamLive=function(r){_udl(r);if(r&&r.F!=null){window.__liveF=r.F;siagaRender(r.F);}};document.addEventListener("DOMContentLoaded",function(){siagaRender(window.__liveF==null?null:window.__liveF);});
function alarmZone(f){return f<=0.64?0:f<=1.86?1:f<=2.55?2:f<=3.40?3:4;}
const ALARM_META={1:{n:"SIAGA IV",c:"#38BDF8",d:"1.2s"},2:{n:"SIAGA III",c:"#FBBF24",d:"0.8s"},3:{n:"SIAGA II",c:"#F97316",d:"0.5s"},4:{n:"SIAGA I",c:"#EF4444",d:"0.28s"}};
let alarmState={zone:0,sound:false,notif:false,acked:false,loop:null,ctx:null};
function alarmBanner(){let b=document.getElementById("alarm-banner");if(!b){b=document.createElement("div");b.id="alarm-banner";document.body.appendChild(b);}return b;}
function beep(f0,f1,dur,vol,type){const c=alarmCtx();if(!c)return;const o=c.createOscillator(),g=c.createGain();o.type=type||"sine";o.frequency.setValueAtTime(f0,c.currentTime);if(f1)o.frequency.exponentialRampToValueAtTime(f1,c.currentTime+dur);g.gain.setValueAtTime(vol,c.currentTime);g.gain.exponentialRampToValueAtTime(0.0001,c.currentTime+dur);o.connect(g);g.connect(c.destination);o.start();o.stop(c.currentTime+dur);}
function alarmLoopStop(){if(alarmState.loop){clearInterval(alarmState.loop);alarmState.loop=null;}}
function alarmLoopStart(z){alarmLoopStop();if(!alarmState.sound)return;const run=function(){if(z===1){beep(880,0,0.18,0.25);setTimeout(function(){beep(880,0,0.18,0.25);},260);}else if(z===2){[0,220,440].forEach(function(t){setTimeout(function(){beep(1046,0,0.15,0.3);},t);});}else if(z===3){
beep(660,880,0.45,0.35,"square");setTimeout(function(){beep(880,660,0.45,0.35,"square");},500);}else{beep(1200,500,0.35,0.45,"sawtooth");setTimeout(function(){beep(500,1200,0.35,0.45,"sawtooth");},380);}};run();alarmState.loop=setInterval(run,z===1?4000:z===2?3000:z===3?1000:760);}
function alarmNotify(z,meta,f){if(!alarmState.notif||!("Notification" in window)||Notification.permission!=="granted")return;try{new Notification("BENDUNG CIAWI - "+meta.n,{body:"TMA outlet "+(f==null?"-":f.toFixed(2).replace(".",","))+" m melewati ambang siaga. Periksa konduit & kondisi hilir.",tag:"ciawi-siaga-"+z});}catch(e){}}
function alarmPanel(){let p=document.getElementById("alarm-panel");if(p)return p;p=document.createElement("div");p.id="alarm-panel";p.innerHTML='<button id="al-test">UJI SIRINE</button><button id="al-sound">SUARA: OFF</button><button id="al-notif">NOTIF: OFF</button>';document.body.appendChild(p);
document.getElementById("al-sound").onclick=function(){alarmState.sound=!alarmState.sound;this.textContent="SUARA: "+(alarmState.sound?"ON":"OFF");alarmCtx();if(alarmState.sound&&alarmState.zone>0&&!alarmState.acked)alarmLoopStart(alarmState.zone);if(!alarmState.sound)alarmLoopStop();};
document.getElementById("al-notif").onclick=function(){if(!("Notification" in window)){this.textContent="NOTIF: N/A";return;}const nb=this;if(Notification.permission==="granted"){alarmState.notif=!alarmState.notif;nb.textContent="NOTIF: "+(alarmState.notif?"ON":"OFF");}else{Notification.requestPermission().then(function(pr){alarmState.notif=pr==="granted";nb.textContent="NOTIF: "+(alarmState.notif?"ON":"OFF");});}};
document.getElementById("al-test").onclick=function(){const zs=[1,2,3,4],fs=[0.8,2.0,2.8,3.6];let i=0;const step=function(){if(i>=zs.length){alarmCheck(window.__liveF==null?null:window.__liveF);return;}alarmCheck(fs[i],zs[i]);i++;setTimeout(step,4000);};step();};return p;}
function alarmCheck(f,forceZone){const b=alarmBanner();const z=forceZone!=null?forceZone:(f==null?0:alarmZone(f));if(z===0){b.style.display="none";if(alarmState.zone!==0){alarmState.zone=0;alarmState.acked=false;alarmLoopStop();}return;}
const meta=ALARM_META[z];const changed=z!==alarmState.zone;alarmState.zone=z;if(changed)alarmState.acked=false;b.style.display="flex";b.style.borderColor=meta.c;b.style.color=meta.c;b.style.animation="alarmBlink "+meta.d+" infinite";
b.innerHTML='<span style="font-size:16px">⚠</span><span>'+meta.n+' - TMA OUTLET '+(f==null?"":f.toFixed(2).replace(".",",")+" m")+' MELEWATI AMBANG SIAGA</span><button id="al-ack" style="background:none;border:1px solid '+meta.c+';color:'+meta.c+';border-radius:6px;padding:2px 8px;font-weight:700;cursor:pointer">✋ BISU</button>';
const ack=document.getElementById("al-ack");if(ack)ack.onclick=function(){alarmState.acked=true;alarmLoopStop();};if(changed){alarmNotify(z,meta,f);alarmLoopStart(z);}else if(!alarmState.acked&&!alarmState.loop)alarmLoopStart(z);}
const _udlA=updateDamLive;updateDamLive=function(r){_udlA(r);if(r&&r.F!=null)alarmCheck(r.F);};document.addEventListener("DOMContentLoaded",function(){alarmPanel();alarmCheck(window.__liveF==null?null:window.__liveF);});
function toast(msg,type){let st=document.getElementById("toast-stack");if(!st){st=document.createElement("div");st.id="toast-stack";document.body.appendChild(st);}const col={success:"#35e0a1",error:"#f43f5e",warn:"#fbbf24",info:"#4cd7f6"}[type||"info"];const t=document.createElement("div");t.className="toast";t.style.borderColor=col;t.style.color=col;t.textContent=msg;st.appendChild(t);setTimeout(function(){t.style.transition="opacity .4s";t.style.opacity="0";setTimeout(function(){t.remove();},420);},4500);}
function skeletonShow(){if(document.getElementById("skeleton-layer"))return;const l=document.createElement("div");l.id="skeleton-layer";l.innerHTML='<div class="sk" style="height:56px"></div><div class="sk" style="height:230px"></div><div class="sk" style="height:38px"></div><div class="sk" style="height:38px"></div><div class="sk" style="height:38px"></div><div class="sk" style="height:160px"></div>';document.body.appendChild(l);}
function skeletonHide(){const l=document.getElementById("skeleton-layer");if(!l)return;l.style.opacity="0";setTimeout(function(){l.remove();},450);}skeletonShow();window.addEventListener("load",function(){setTimeout(skeletonHide,250);});setTimeout(skeletonHide,3500);
const _expCsv=exportTableToCSV;exportTableToCSV=function(fn){_expCsv(fn);toast("Ekspor CSV sukses: "+fn,"success");};
document.addEventListener("DOMContentLoaded",function(){const n=(typeof REAL_DATA!=="undefined"&&REAL_DATA)?REAL_DATA.length:0;let last=0;try{last=+localStorage.getItem("ciawi-last-count")||0;}catch(e){}if(last&&n>last){toast("Import sukses: +"+(n-last)+" rekaman baru (total "+n.toLocaleString("id-ID")+")","success");}else if(n>0){toast("Data NERACA termuat: "+n.toLocaleString("id-ID")+" rekaman","info");}try{localStorage.setItem("ciawi-last-count",String(n));}catch(e){}});
let proxyState="on";const _cw=console.warn;console.warn=function(){const s=Array.prototype.join.call(arguments," ");if(s.indexOf("Live telemetry offline")===0&&proxyState!=="off"){proxyState="off";toast("Proxy/telemetri putus - dashboard mode offline","error");}_cw.apply(console,arguments);};
const _udlT=updateDamLive;updateDamLive=function(r){_udlT(r);if(proxyState==="off"){proxyState="on";toast("Telemetri LIVE pulih - data mengalir kembali","success");}};

function trendSvg(){if(window.__tsvg2&&document.contains(window.__tsvg2))return window.__tsvg2;let best=null,bn=0;document.querySelectorAll("svg").forEach(function(sv){if(sv.querySelector("[id^=dam-],#wl-live-line,#tma-out-label"))return;const c=sv.querySelectorAll("circle").length;if(c>bn){bn=c;best=sv;}});window.__tsvg2=best;return best;}


// --- ekspor agregat per jam (patch 2026-09-24) ---
function exportHourlyCSV(filename){
  const src = workingDataset || [];
  if (!src.length) { alert("Tidak ada data pada rentang aktif untuk diagregasi per jam."); return; }
  const buckets = new Map();
  src.forEach(r => {
    const k = (r.tanggal || "") + " " + String(r.jam || "00:00").slice(0, 2);
    if (!buckets.has(k)) buckets.set(k, []);
    buckets.get(k).push(r);
  });
  const num = v => { if (v == null || v === "" || v === "-") return null; const x = parseFloat(String(v).replace(",", ".")); return Number.isFinite(x) ? x : null; };
  const agg = [...buckets.entries()].sort((a, b) => (a[0] < b[0] ? -1 : 1)).map((e, idx) => {
    const g = e[1];
    const out = { no: idx + 1, tanggal: e[0].slice(0, 10), jam: e[0].slice(11, 13) + ":00" };
    Object.keys(g[0]).forEach(key => {
      if (key === "no" || key === "tanggal" || key === "jam") return;
      const vals = g.map(r => num(r[key])).filter(x => x !== null);
      if (vals.length) { out[key] = Math.round((vals.reduce((a, b) => a + b, 0) / vals.length) * 100) / 100; }
      else { out[key] = g[0][key]; }
    });
    return out;
  });
  const old = workingDataset;
  workingDataset = agg;
  try { exportTableToCsv(filename || "neraca_per_jam.csv"); } finally { workingDataset = old; }
}
